...
# גרסא / מזהה שינויים (אם יש git בסביבת הריצה)
def get_git_info():
    try:
        commit = (
...
def save_trade(trade_date, symbol, action, price, qty, profit_usd=0, profit_ils=0):
    ...
    try:
        ...
        logger.exception("שגיאה בכתיבת יומן המסחר: %s", e)
        st.warning("שגיאה בשמירת היומן — בדוק הרשאות כתיבה בשרת.")
...
def get_data(symbol: str):
    """
    מחזיר (df, company_name) — רק ערכים serializable.
    אם יש כשל מחזיר (None, None).
    """
    try:
...
def analyze_indicators(df: pd.DataFrame) -> dict:
    """
    מקבל DataFrame ומחזיר ניתוח טכני מסוכם (score/recommendation/notes).
    """
    try:
        res = {"notes": []}
        score = 50
        ...
        if sma50 is not None and sma200 is not None:
            if sma10 > sma50 > sma200:
                score += 15
                res["notes"].append(
                    "SMA קצר עולה מעל SMA50 ו‑SMA200 — מגמה חיובית ברורה לטווח הבינוני/ארוך."
                )
            elif sma10 > sma50:
                score += 5
        ...
        if rsi is not None:
            if rsi > 70:
                score -= 10
                res["notes"].append(f"RSI גבוה ({rsi:.1f}) — סיכון קניית יתר; שקול הקטנת חשיפה.")
            elif rsi < 30:
                score += 10
                res["notes"].append(f"RSI נמוך ({rsi:.1f}) — ייתכן הזדמנות קניה.")
            else:
        ...
        if macd_val is not None and signal_val is not None:
            if prev_macd is not None and prev_signal is not None:
                if macd_val > signal_val and prev_macd <= prev_signal:
                    score += 8
                    res["notes"].append("חציית MACD למעלה — אות שוורי.")
                elif macd_val < signal_val and prev_macd >= prev_signal:
                    score -= 8
                    res["notes"].append("חציית MACD למטה — אות דובי.")
                else:
        ...
        # ניסיון לקרוא דוחות קרובים (אם קיימים)
        upcoming = []
        try:
            ticker_symbol = df.attrs.get("symbol", "")
            if ticker_symbol:
            ...
            logger.debug("לא הצלחנו לאחזר אירועים קרובים")
        ...
        logger.exception("שגיאה בניתוח אינדיקטורים: %s", e)
        return {"score": 50, "recommendation": "המתן", "notes": ["שגיאה בניתוח"]}


# --- ממשק המשתמש (כולל טיפול בשגיאות להצגה) ---
...
def main():
    ...
    # קריאת נתונים - שימו לב: get_data מחזיר (df, company_name)
    df, company_name = get_data(symbol_input)

    if df is not None:
        # שמירת סימול ביחס ל‑df לשימוש בניתוחים עתידיים
        ...
        tab1, tab2, tab3, tab4, tab5 = st.tabs(
            ["📈 גרף טכני", "🧠 ניתוח חכם", "📰 חדשות", "📓 יומן מסחר", "ℹ️ מידע/דיבאג"]
        )

        # לשונית גרף
        with tab1:
            st.caption("רזולוציות נתונים קבועות — אין שינוי רזולוציית נרות חופשית.")
            ...
            fig.update_layout(height=650, template="plotly_white" if not dark else "plotly_dark", xaxis_rangeslider_visible=False, hovermode="x unified")
        ...
        with tab2:
            ...
            if analysis.get("upcoming_reports"):
                ...
                try:
                    # ניסיון נוסף להציג calendar מה‑yfinance
                    t = yf.Ticker(symbol_input)
                    cal = getattr(t, "calendar", None)
                    if isinstance(cal, pd.DataFrame) and not cal.empty:
                        st.write(cal)
                    else:
        ...
        # לשונית חדשות — צור Ticker כאן (לא להחזיר ממטמון!)
        with tab3:
            st.subheader(f"חדשות אחרונות על {symbol_input}")
            try:
                ticker_for_news = yf.Ticker(symbol_input)
                news = getattr(ticker_for_news, "news", None)
                if news:
                    for item in news[:6]:
        ...
        # לשונית דיבאג/גרסה
        with tab5:
            st.subheader("מידע דיבאג / גרסה")
            st.write(f"Allowed ranges: {ALLOWED_RANGES}")
            st.write(f"Symbol: {symbol_input}")
            st.write(f"Data rows: {len(df)}")
            try:
                if os.path.exists("CHANGELOG.md"):
                    with open("CHANGELOG.md", "r", encoding="utf-8") as f:
                        changelog = f.read()
                    st.code("\n".join(changelog.splitlines()[:50]), language="markdown")
                else:
...
if __name__ == "__main__":
    try:
        ...
        logger.exception("Unhandled exception in app: %s", e)
        # הצגת traceback ב‑UI (Streamlit Cloud יציג גם בלוגים)
        st.error("אירעה שגיאה בלתי צפויה — ראה פירוט למטה.")
        st.code(tb)
