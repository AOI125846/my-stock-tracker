...
def load_journal():
    if not os.path.exists(JOURNAL_FILE):
        df = pd.DataFrame(
            columns=["תאריך", "סימול", "פעולה", "מחיר ($)", "כמות", "רווח ($)", "רווח (₪)"]
        )
...
def save_trade(d, symbol, action, price, qty, usd_rate):
    profit_usd = price * qty if action == "מכירה" else 0.0
...
def get_data(symbol):
    try:
        ...
        logger.exception("שגיאה בטעינת נתונים: %s", e)
        return None
...
def analyze(df):
    ...
    if last["SMA10"] > last["SMA50"] > last["SMA200"]:
        score += 15
        notes.append("מגמה חיובית חזקה (SMA10 > SMA50 > SMA200)")
...
def main():
    ...
    with col2:
        st.metric("שער דולר", f"₪{usd_rate:.2f}")
    ...
    with tab3:
        ...
        price = c2.number_input("מחיר ($)", value=float(last_price))
        ...
        # ===== ייצוא Excel ללא xlsxwriter =====
        output = io.BytesIO()
        journal.to_excel(output, index=False)
        output.seek(0)

        st.download_button(
            "📥 הורדת יומן מסחר (Excel)",
            data=output,
